...
public class RetrievalService {
    ...
    public RetrievalOutcome retrieveOutcome(String query) {
        ...
        try {
            ...
            return handleVectorSearchFailureOutcome(
                    ...
                    query,
                    notices,
                    props.getRag().getSearchTopK(),
                    props.getRag().getSearchReturnK());
        }

        List<Document> reranked =
                rerankWithDiagnostics(query, docs, props.getRag().getSearchReturnK(), notices);
    ...
    public RetrievalOutcome retrieveWithLimitOutcome(String query, int maxDocs, int maxTokensPerDoc) {
        ...
        List<Document> docs;
        try {
            int baseTopK = Math.max(1, Math.max(maxDocs, props.getRag().getSearchTopK()));
            docs = executeVersionAwareSearch(query, boostedQuery, versionFilter, baseTopK);
        } catch (RuntimeException exception) {
    ...
    private List<Document> executeVersionAwareSearch(
            String query, String boostedQuery, Optional<VersionFilterPatterns> versionFilter, int baseTopK) {
        ...
        SearchRequest searchRequest =
                SearchRequest.builder().query(boostedQuery).topK(topK).build();
        ...
        if (versionFilter.isPresent()) {
            ...
            if (versionMatchedDocs.size() >= 2) {
                ...
                log.info(
                        "Insufficient version-specific docs ({}), using all {} candidates",
                        versionMatchedDocs.size(),
                        docs.size());
            }
        }
    ...
    private RetrievalOutcome handleVectorSearchFailureOutcome(
            RuntimeException exception, String query, List<RetrievalNotice> notices, int maxDocs, int returnLimit) {
        ...
        notices.add(new RetrievalNotice("Vector search failed; using keyword fallback", describeFailure(exception)));
        ...
        if (outcome.isFailed()) {
            ...
            notices.add(new RetrievalNotice("Keyword fallback failed; returning empty results", outcomeMessage));
            return new RetrievalOutcome(List.of(), notices);
        }

        log.info("Local keyword search returned {} hits", outcome.hits().size());

        List<Document> fallbackDocs = outcome.hits().stream()
                .map(hit -> {
    ...
    private RetrievalOutcome executeFallbackSearchOutcome(
            String query, int maxDocs, RuntimeException exception, List<RetrievalNotice> notices) {
    ...
    private List<Document> rerankWithDiagnostics(
            String query, List<Document> docs, int returnLimit, List<RetrievalNotice> notices) {
        List<Document> uniqueByUrl = docs.stream()
                .collect(Collectors.toMap(
                        doc -> String.valueOf(doc.getMetadata().get(METADATA_URL)), doc -> doc, (first, dup) -> first))
                .values()
                .stream()
                .collect(Collectors.toList());
        ...
        try {
            ...
            notices.add(new RetrievalNotice("Reranking failed; using original order", describeFailure(exception)));
