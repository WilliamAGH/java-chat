...
/**
 * Bounds whole dense-search concurrency within the retrieval stage deadline.
 */
final class QdrantSearchAdmission {
    ...
    <T> T execute(
        ...
        Duration requiredQueryTimeout = Objects.requireNonNull(queryTimeout, "queryTimeout");
        ...
        long queryDeadlineNanos = Math.min(stageDeadlineNanos, System.nanoTime() + requiredQueryTimeout.toNanos());
        ...
        if (!admitted) {
            TimeoutException admissionTimeout =
                    new TimeoutException("Qdrant search admission exhausted the shared retrieval query budget");
            throw admissionFailure(
                    requiredCollectionNames,
                    "AdmissionTimeout",
