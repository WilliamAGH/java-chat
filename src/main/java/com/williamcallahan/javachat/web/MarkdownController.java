...
public class MarkdownController {
    ...
    public ResponseEntity<MarkdownRenderResponse> renderMarkdown(@RequestBody MarkdownRenderRequest renderRequest) {
        try {
            ...
            logger.debug(
                    "Processing markdown of length: {}", renderRequest.content().length());
            ...
            logger.error(
                    "Error rendering markdown (exception type: {})",
                    renderException.getClass().getSimpleName());
            return ResponseEntity.status(500)
                    .body(new MarkdownErrorResponse(
                            "Failed to render markdown", exceptionBuilder.describeException(renderException)));
        }
    }
    ...
    public ResponseEntity<MarkdownRenderResponse> previewMarkdown(@RequestBody MarkdownRenderRequest renderRequest) {
        try {
            ...
            logger.error(
                    "Error rendering preview markdown (exception type: {})",
                    previewException.getClass().getSimpleName());
            return ResponseEntity.status(500)
                    .body(new MarkdownErrorResponse(
                            "Failed to render preview", exceptionBuilder.describeException(previewException)));
        }
    }
    ...
    public ResponseEntity<MarkdownCacheStatsResponse> getCacheStats() {
        try {
            ...
            logger.error(
                    "Error getting cache stats (exception type: {})",
                    statsException.getClass().getSimpleName());
            return ResponseEntity.status(500)
                    .body(new MarkdownErrorResponse(
                            "Failed to get cache stats", exceptionBuilder.describeException(statsException)));
        }
    }
    ...
    public ResponseEntity<MarkdownCacheClearResponse> clearCache() {
        try {
            ...
            return ResponseEntity.ok(new MarkdownCacheClearOutcome("success", "Cache cleared successfully"));

        } catch (RuntimeException clearException) {
            logger.error(
                    "Error clearing cache (exception type: {})",
                    clearException.getClass().getSimpleName());
            return ResponseEntity.status(500)
                    .body(new MarkdownCacheClearOutcome(
                            "error", "Failed to clear cache: " + exceptionBuilder.describeException(clearException)));
        }
    }
    ...
    public ResponseEntity<MarkdownStructuredResponse> renderStructured(
        ...
        try {
            ...
            logger.debug(
                    "Processing markdown with UnifiedMarkdownService, length: {}",
                    renderRequest.content().length());
            ...
            logger.error(
                    "Error rendering structured markdown (exception type: {})",
                    structuredException.getClass().getSimpleName());
            return ResponseEntity.status(500)
                    .body(new MarkdownStructuredErrorResponse(
