...
public class SystemPromptConfig {
    ...
    static final String JAVA_FENCE_VALIDITY_CLAUSE =
            "Put every multi-line Java example entirely inside one fenced `java` block. Never emit part of a declaration or method as prose. Finish the complete example before an enrichment marker; any fenced block after that marker must be a separate, complete example. Never split one example across multiple fences. Parameterize generic declarations, signatures, casts, and constructor types; never emit a raw type, while retaining idiomatic diamond inference and legal generic class literals. Before emitting the block, perform a final internal consistency check of its delimiters, quotes, declarations, imports, type arguments, and referenced API signatures. The block must contain syntactically valid Java that compiles with its stated context. A standalone program must have a launchable entry point appropriate to the active Java release, for example `public static void main(String[] args)`; never invent an API.";
