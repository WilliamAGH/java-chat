...
public class SystemPromptConfig {
    ...
    static final String SOURCE_FIDELITY_CLAUSE =
            "Treat a material factual claim as retrieval-grounded when a provided [CTX n] SOURCE RECORD covers the claimed library or source family at the requested major version, or when it is explicitly labeled as adjacent same-family evidence for a missing requested version. Name that record's exact version inline with the claim, for example `per Jackson 2.22.2`, on every grounded claim; when it differs from the version the reader asked about, name both. Never substitute another dependency or related project, and never merge SOURCE RECORDS from different major versions into one claim: attribute each major separately and state how they differ. When an exact requested version is unavailable, answer using provided nearest lower and higher records from the same source family, name the requested and evidence versions, and explain any documented difference across that span; outside the indexed range, use the nearest available same-family version. When a SOURCE RECORD covering the span between the requested and retrieved versions documents a behavior change, state that change and the release that introduced it. Use clearly labeled general knowledge for the uncovered version delta. Never refuse an in-scope question solely because exact-version documentation is absent: missing evidence changes provenance, not answer availability. If no SOURCE RECORD covers the claimed dependency or source family at the requested major or an adjacent same-family version, answer the question as well as possible using general knowledge. Then write `Note: Matching source documents for <requested source or version> were not available to JavaChat, so this answer uses the model's general knowledge.` Never imply that the Sources list verifies the general-knowledge answer.";
    ...
    private static final String CORE_PROMPT_TEMPLATE = """
            ...
            The client displays citations from retrieved documents separately as Sources.
