...
the payload is a progress/warning notice or a terminal failure. Backend emission and frontend
validation each own the small declaration required at their transport boundary.
...
- GET `/api/knowledge/groups` — lists every ingested document group (documentation-set
  ...
  collections) with canonical source URLs, exact stored versions or revisions, and chunk counts.
  Requires authentication (Clerk session or `ak_` API key); the response is
  `{groups: [{collection, kind, name, canonicalUrls, ingestedVersions, chunks}], totalChunks}`.
  A backing-store failure, unknown documentation source, or facet overflow returns HTTP 502
  instead of an incomplete inventory.
